...
CREATE TABLE IF NOT EXISTS notification (
                                            ...
                                            user_id BIGINT NULL COMMENT '멤버 ID (FK)',
...
SET @sql = IF(@member_id_exists > 0 AND @user_id_exists = 0,
    'ALTER TABLE notification CHANGE COLUMN member_id user_id BIGINT NULL COMMENT "멤버 ID (FK)";',
...
SET @sql = IF(@user_id_exists_after = 0,
    'ALTER TABLE notification ADD COLUMN user_id BIGINT NULL COMMENT "멤버 ID (FK)" AFTER id;',
...
SET @sql = IF(@type_exists = 0,
    'ALTER TABLE notification ADD COLUMN type VARCHAR(50) NULL COMMENT "알림 타입" AFTER user_id;',
...
-- 2-4. content 컬럼 확인 및 TEXT 타입으로 설정
SET @content_exists = (
    SELECT COUNT(*)
    FROM information_schema.columns
    WHERE table_schema = DATABASE()
...
-- 2-6. BaseEntity 필드들 (created_at, updated_at, deleted_at) 확인 및 추가
...
-- member 테이블이 존재하고 외래키가 없다면 추가
SET @sql = IF(@member_table_exists > 0 AND @fk_exists = 0,
    ...
    'SELECT "notification foreign key already exists or member table not found" as result;'
);
